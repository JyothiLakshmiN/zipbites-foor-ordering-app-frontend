...
import { getMyUserProfile } from "@/api/MyUserApi";
...
const CheckoutButton = ({onCheckout, disabled}: Props) => {
    const {isAuthenticated, isLoading: isAuthLoading, loginWithRedirect, isLoading} = useAuth0();

    const { pathname } = useLocation();

    const {currentUser, isPending: isGetUserLoading }= getMyUserProfile();
    const onLogin = async() => {
